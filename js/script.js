window.onload = function()
{
    ...
    */
    ...
    var puntoDeVista = function()
	{
        ...
        for (var i = 0; i < planetas.length; i++) {
            if(other < planetas.length){
                other++;
            }else{
                other=0;
            }
            if(frustum.intersectsObject(planetas[i].objeto) && frustum.intersectsObject(planetas[other].objeto)){
                        if(!planetas[i].vista){
                            planetas[i].vista = true;
                            responsiveVoice.speak("Estas viendo "+planetas[i].nombre +" y "+planetas[i+1].nombre, "Spanish Female");
                        }
                    }else{
